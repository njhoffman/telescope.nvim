...
local debounce =
  require 'telescope.debounce'
local deprecated =
  require 'telescope.deprecated'
local log =
  require 'telescope.log'
local mappings =
...
local p_highlighter =
  require 'telescope.pickers.highlights'
local p_scroller =
  require 'telescope.pickers.scroller'
local p_window =
  require 'telescope.pickers.window'
local Layout =
  require 'telescope.pickers.layout'

local EntryManager =
  require 'telescope.entry_manager'
local MultiSelect =
...
local ns_telescope_matching =
  a.nvim_create_namespace 'telescope_matching'
local ns_telescope_prompt =
  a.nvim_create_namespace 'telescope_prompt'
...
--
---@param picker Picker
local function default_create_layout(
  picker
)
  ...
  local layout =
    Layout {
      ...
      mount = function(
        ...
        local popup_opts =
          picker:get_window_options(
            vim.o.columns,
            line_count
          )

        -- `popup.nvim` massaging so people don't have to remember minheight shenanigans
        popup_opts.results.minheight =
          popup_opts.results.height
        popup_opts.results.highlight =
          'TelescopeResultsNormal'
        popup_opts.results.borderhighlight =
          'TelescopeResultsBorder'
        popup_opts.results.titlehighlight =
          'TelescopeResultsTitle'
        popup_opts.prompt.minheight =
          popup_opts.prompt.height
        popup_opts.prompt.highlight =
          'TelescopePromptNormal'
        popup_opts.prompt.borderhighlight =
          'TelescopePromptBorder'
        popup_opts.prompt.titlehighlight =
          'TelescopePromptTitle'
        if
          popup_opts.preview
        then
          popup_opts.preview.minheight =
            popup_opts.preview.height
          popup_opts.preview.highlight =
            'TelescopePreviewNormal'
          popup_opts.preview.borderhighlight =
            'TelescopePreviewBorder'
          popup_opts.preview.titlehighlight =
            'TelescopePreviewTitle'
        end

        local results_win, results_opts =
          picker:_create_window(
            '',
            popup_opts.results
          )
        local results_bufnr =
        ...
        if
          popup_opts.preview
        then
          local preview_win, preview_opts =
            picker:_create_window(
              '',
              popup_opts.preview
            )
      ...
      unmount = function(
        ...
        utils.win_delete(
          'prompt_border_win',
          self.prompt.border.winid,
          true,
          true
        )
        utils.win_delete(
          'results_border_win',
          self.results.border.winid,
          true,
          true
        )
        if
      ...
      update = function(
        ...
        local results_win =
          self.results.winid
        local preview_win = self.preview
          and self.preview.winid
        ...
        then
          ...
          then
            ...
            if
              ...
              and picker.previewer.state.winid
            then
...
function Picker:new(
  ...
  if
    opts.layout_strategy
    and opts.get_window_options
  then
  ...
  local obj =
    setmetatable(
      {
        prompt_title = vim.F.if_nil(
          opts.prompt_title,
          config.values.prompt_title
        ),
        results_title = vim.F.if_nil(
          opts.results_title,
          config.values.results_title
        ),
        -- either whats passed in by the user or whats defined by the previewer
        preview_title = opts.preview_title,

        prompt_prefix = vim.F.if_nil(
          opts.prompt_prefix,
          config.values.prompt_prefix
        ),
        wrap_results = vim.F.if_nil(
          opts.wrap_results,
          config.values.wrap_results
        ),
        selection_caret = vim.F.if_nil(
          opts.selection_caret,
          config.values.selection_caret
        ),
        entry_prefix = vim.F.if_nil(
          opts.entry_prefix,
          config.values.entry_prefix
        ),
        multi_icon = vim.F.if_nil(
          opts.multi_icon,
          config.values.multi_icon
        ),

        initial_mode = vim.F.if_nil(
          opts.initial_mode,
          config.values.initial_mode
        ),
        _original_mode = vim.api.nvim_get_mode().mode,
        debounce = vim.F.if_nil(
          tonumber(
            opts.debounce
          ),
          nil
        ...
        get_status_text = vim.F.if_nil(
          opts.get_status_text,
        ...
        current_previewer_index = opts.current_previewer_index
          or 1,

        default_selection_index = opts.default_selection_index,

        get_selection_window = vim.F.if_nil(
          opts.get_selection_window,
          config.values.get_selection_window
        ),

        cwd = opts.cwd,

        _find_id = 0,
        _completion_callbacks = type(
          opts._completion_callbacks
        )
              == 'table'
            and opts._completion_callbacks
          or {},
        manager = (
          type(
              opts.manager
        ...
        file_ignore_patterns = vim.F.if_nil(
          opts.file_ignore_patterns,
          config.values.file_ignore_patterns
        ),

        scroll_strategy = vim.F.if_nil(
          opts.scroll_strategy,
          config.values.scroll_strategy
        ),
        sorting_strategy = vim.F.if_nil(
          opts.sorting_strategy,
          config.values.sorting_strategy
        ),
        tiebreak = vim.F.if_nil(
          opts.tiebreak,
          config.values.tiebreak
        ),
        selection_strategy = vim.F.if_nil(
          opts.selection_strategy,
          config.values.selection_strategy
        ),

        push_cursor_on_edit = vim.F.if_nil(
          opts.push_cursor_on_edit,
          false
        ),
        push_tagstack_on_edit = vim.F.if_nil(
          opts.push_tagstack_on_edit,
          false
        ),

        layout_strategy = layout_strategy,
        layout_config = config.smarter_depth_2_extend(
          opts.layout_config
            or {},
        ...
        __cycle_layout_list = vim.F.if_nil(
          opts.cycle_layout_list,
          config.values.cycle_layout_list
        ),
          ...
          winblend = vim.F.if_nil(
            ...
            type(
              opts.window
                  ...
                  == 'table'
                and opts.window.winblend
              or config.values.winblend
          ),
          border = vim.F.if_nil(
            opts.border,
            type(
              opts.window
                  ...
                  == 'table'
                and opts.window.border
              or config.values.border
          ),
          borderchars = vim.F.if_nil(
            ...
            type(
              opts.window
                  ...
                  == 'table'
                and opts.window.borderchars
              or config.values.borderchars
          ),
        },

        cache_picker = config.resolve_table_opts(
          opts.cache_picker,
          vim.deepcopy(
            config.values.cache_picker
          )
        ),
  ...
  obj.create_layout = opts.create_layout
    or config.values.create_layout
    or default_create_layout
  obj.get_window_options = opts.get_window_options
    or p_window.get_window_options

  if
    obj.all_previewers
      ~= nil
    and obj.all_previewers
      ~= false
  then
    if
      obj.all_previewers[1]
      == nil
    then
      obj.all_previewers =
        {
    ...
    obj.previewer =
      obj.all_previewers[obj.current_previewer_index]
    if
      obj.preview_title
        == nil
      or #obj.all_previewers > 1
      or opts.resumed_picker
        and opts.fix_preview_title ~= true
    then
      obj.preview_title =
        obj.previewer:title(
          nil,
          config.values.dynamic_preview_title
        )
    else
      obj.fix_preview_title =
        true
    end
  else
    obj.previewer =
  ...
  local __hide_previewer =
    opts.__hide_previewer
  if
    __hide_previewer
  then
    obj.hidden_previewer =
      obj.previewer
    obj.previewer =
      nil
  else
  ...
  then
    ...
    do
      obj:register_completion_callback(
        on_complete_item
      )
    end
  end
...
function Picker:clear_extra_rows(
  results_bufnr
)
  if
    self:is_done()
  ...
  then
    log.debug(
      'Invalid results_bufnr for clearing:',
      results_bufnr
    )
    return
  end
...
function Picker:highlight_one_row(
  results_bufnr,
  prompt,
  display,
  row
  ...
  if
    not self.sorter.highlighter
  then
    return
  end

  local highlights =
    self.sorter:highlighter(
      prompt,
      display
    )
...
---@return boolean
function Picker:can_select_row(
  row
)
  if
    self.sorting_strategy
  ...
  then
    ...
    return row
        >= 0
      and row <= self.max_results
      and row
        >= self.max_results - self.manager:num_results()
  end
end
...
function Picker:_create_window(
  bufnr,
  popup_opts
)
  local what = bufnr
  ...
  local border_win = opts
    and opts.border
    and opts.border.win_id
  if
    border_win
  then
    a.nvim_win_set_option(
...
function Picker:find(
  ...
  self.results_win, self.results_bufnr, self.results_border =
    layout.results.winid,
    layout.results.bufnr,
    layout.results.border
  if
    layout.preview
  then
  ...
  local status_updater =
    self:get_status_updater(
      self.prompt_win,
      self.prompt_bufnr
    )
  local debounced_status =
    debounce.throttle_leading(
      status_updater,
      50
    )
  ...
  then
    ...
    utils.notify(
      'pickers.find',
      {
        msg = "`initial_mode` should be one of ['normal', 'insert'] but passed "
          .. self.initial_mode,
        level = 'ERROR',
      }
    )
  ...
  local main_loop = async.void(
    function(
      ...
      do
        ...
        local prompt =
          self:_get_next_filtered_prompt()

        -- TODO: Entry manager should have a "bulk" setter. This can prevent a lot of redraws from display
        if
          self.cache_picker
            == false
          or self.cache_picker.is_cached
            ~= true
        then
          self.sorter:_start(
            prompt
    ...
    setmetatable(
      {
        ...
        preview_border_win = self.preview_border
          and self.preview_border.winid,
      },
      {
        __mode = 'kv',
      }
...
function Picker:delete_selection(
  ...
  local original_selection_strategy =
    self.selection_strategy
  self.selection_strategy =
    'row'

  local delete_selections =
    self._multi:get()
  local used_multi_select =
    true
  if
    vim.tbl_isempty(
      delete_selections
  ...
  do
    local delete_cb_return =
      delete_cb(
        self.finder.results[index]
      )
    if
      delete_cb_return
        == nil
  ...
  vim.defer_fn(
    function(
      ...
      self.selection_strategy =
        original_selection_strategy
    end,
    50
  )
end

function Picker:set_prompt(
  text
)
  self:reset_prompt(
    text
...
function Picker.close_windows(
  status
)
  local prompt_bufnr =
    status.layout.prompt.bufnr
...
---@param change number
function Picker:move_selection(
  change
)
  self:set_selection(
    self:get_selection_row()
      + change
  )
end
...
---@param row number: the number of the chosen row
function Picker:remove_selection(
  row
)
  local entry =
    self.manager:get_entry(
...
function Picker:is_multi_selected(
  entry
)
  return self._multi:is_selected(
    entry
  )
end
...
function Picker:_reset_prefix_color(
  hl_group
)
  self._current_prefix_hl_group = hl_group
    or nil

  if
    self.prompt_prefix
      ~= ''
    and a.nvim_buf_is_valid(
      self.prompt_bufnr
    )
  then
    vim.api.nvim_buf_add_highlight(
...
function Picker:change_prompt_prefix(
  new_prefix,
  hl_group
)
  if
...
function Picker:reset_prompt(
  ...
  local prompt_text = self.prompt_prefix
    .. (
      text
      or ''
    )
...
function Picker:refresh(
  ...
  then
    local handle = type(
      opts.new_prefix
    ) == 'table' and unpack or function(
      x
    )
      return x
    end
    self:change_prompt_prefix(
      handle(
        opts.new_prefix
      ),
      opts.prefix_hl_group
    )
  end
...
function Picker:set_selection(
  ...
  local set_ok, set_errmsg = pcall(
    function(
      ...
      self.highlighter:hi_selection(
        row,
        caret:match '(.*%S)'
      )
      self.highlighter:hi_sorter(
        row,
        prompt,
        display
      )
...
function Picker:update_prefix(
  ...
  then
    log.trace(
      string.format(
        'no line found at row %d in buffer %d',
        row,
        self.results_bufnr
      )
    )
  ...
  then
    log.warn(
      string.format(
        "can't identify old caret in line: %s",
        line
      )
    )
    return
...
function Picker:refresh_previewer(
  ...
  if
    ...
    and a.nvim_win_is_valid(
      status.layout.preview.winid
    )
  then
    ...
    then
      if
        self.fix_preview_title
      then
        return
      end

      local new_title =
        self.previewer:title(
          self._selection_entry,
          config.values.dynamic_preview_title
        )
      if
      ...
      then
        self.preview_title =
          new_title
        self.layout.preview.border:change_title(
          new_title
        )
      end
    end
...
function Picker:cycle_previewers(
  next
)
  local size =
    #self.all_previewers
  ...
  self.current_previewer_index = self.current_previewer_index
    + next
  if
    self.current_previewer_index
    > size
  then
    self.current_previewer_index =
      1
  ...
  then
    self.previewer =
      self.all_previewers[self.current_previewer_index]
    self:refresh_previewer()
  elseif
    self.hidden_previewer
  then
    self.hidden_previewer =
      self.all_previewers[self.current_previewer_index]
  end
...
function Picker:_reset_track(
  ...
  self.stats.displayed =
    0
  self.stats.display_fn =
    0
  self.stats.previewed =
    0
  self.stats.status =
    0
...
function Picker:register_completion_callback(
  cb
)
  table.insert(
...
function Picker:close_existing_pickers(
  ...
  in
    ipairs(
      state.get_existing_prompt_bufnrs()
    )
  do
    pcall(
      actions.close,
...
---@return function
function Picker:get_status_updater(
  prompt_win,
  prompt_bufnr
)
  return function(
...
---@return function
function Picker:get_result_processor(
  find_id,
  prompt,
  status_updater
)
...
function Picker:_do_selection(
  ...
  local selection_strategy = self.selection_strategy
    or 'reset'
  ...
  then
    if
      self._selection_row
        == nil
      and self.default_selection_index
        ~= nil
    then
      self:set_selection(
        self:get_row(
...
pickers.new = function(
    ...
    assert(
      type(
        ...
        or type(
          ...
          == 'number',
      'Should be string or number, found: '
        .. type(
          k
        )
    )
  ...
  elseif
    type(
      ...
      == 'table'
    and opts['preview']['hide_on_startup']
  then
    result['__hide_previewer'] =
      true
  end
...
---@param prompt_bufnr number
function pickers.on_close_prompt(
  prompt_bufnr
)
  local status =
    state.get_status(
  ...
  then
    ...
    if
      picker.cache_picker.disabled
      ~= true
    then
      if
        picker.cache_picker.limit_entries
        > 0
      then
        ...
        if
          picker.manager
        then
          picker.manager.linked_states:truncate(
            picker.cache_picker.limit_entries
          )
        else
          picker.manager =
      ...
      picker.default_text =
        picker:_get_prompt()
      picker.cache_picker.selection_row =
        picker._selection_row
      picker.cache_picker.cached_prompt =
        picker:_get_prompt()
      picker.cache_picker.is_cached =
        true
      table.insert(
        cached_pickers,
        1,
      ...
      -- release pickers
      if
        picker.cache_picker.num_pickers
        > 0
      then
        while
          #cached_pickers
...
function pickers.on_resize_window(
  prompt_bufnr
)
  local status =
    state.get_status(
...
function Picker:_get_prompt(
  ...
  local cursor_line = vim.api.nvim_win_get_cursor(
    self.prompt_win
  )[1] - 1
  return vim.api
    .nvim_buf_get_lines(
...
function Picker:_get_next_filtered_prompt(
  ...
  local prompt =
    self:_get_prompt()
  local on_input_result = self._on_input_filter_cb(
    prompt
  ) or {}

  local new_prompt =
    on_input_result.prompt
  if
    new_prompt
  then
    prompt =
  ...
  local new_finder =
    on_input_result.updated_finder
  if
    new_finder
  then
    self.finder:close()
...
function Picker:_resume_picker(
  ...
  if
    self.cache_picker.cached_prompt
    ~= self.default_text
  then
    self:set_prompt(
      self.default_text
    ...
    await_schedule(
      function(
        ...
        if
          self.cache_picker.selection_row
          ~= nil
        then
          self:set_selection(
            self.cache_picker.selection_row
