...
function Picker:new(opts)
  ...
  local obj = setmetatable({
    ...
    _multi = (type(opts._multi) == "table" and getmetatable(opts._multi) == getmetatable(MultiSelect:new())) and
        opts._multi or MultiSelect:new(),
    ...
    window = {
      winblend = vim.F.if_nil(opts.winblend,
      ...
      borderchars = vim.F.if_nil(opts.borderchars,
...
function Picker:recalculate_layout()
  ...
  if popup_opts.preview then
    if preview_win ~= nil then
      ...
      popup_opts.preview.borderhighlight = "TelescopePreviewBorder"
      popup_opts.preview.titlehighlight = "TelescopePreviewTitle"
      local preview_bufnr = status.preview_bufnr ~= nil and vim.api.nvim_buf_is_valid(status.preview_bufnr) and
          status.preview_bufnr or ""
...
function Picker:_reset_prefix_color(hl_group)
  ...
  if self.prompt_prefix ~= "" then
    vim.api.nvim_buf_add_highlight(self.prompt_bufnr, ns_telescope_prompt_prefix,
      self._current_prefix_hl_group or "TelescopePromptPrefix", 0, 0, #self.prompt_prefix)
  end
end
...
function Picker:set_selection(row)
  ...
  if row > a.nvim_buf_line_count(results_bufnr) then
    log.debug(string.format("Should not be possible to get row this large %s %s", row,
...
function Picker:update_prefix(entry, row)
  ...
  local old_caret = string.sub(line, 0, #prefix(true)) == prefix(true) and prefix(true) or
      string.sub(line, 0, #prefix(true, true)) == prefix(true, true) and prefix(true, true) or
      string.sub(line, 0, #prefix(false)) == prefix(false) and prefix(false) or
      string.sub(line, 0, #prefix(false, true)) == prefix(false, true) and prefix(false, true)
